...
void Controller_1()
{
    ...
    if ((Converted_Distance_Left > DESIRED_DISTANCE) | (Converted_Distance_Right > DESIRED_DISTANCE) | (Converted_Distance_Center > DESIRED_DISTANCE))
    {
        int32_t closest_object = Converted_Distance_Left;

        if (closest_object > Converted_Distance_Right) {
            closest_object = Converted_Distance_Right;
        }

        if (closest_object > Converted_Distance_Center) {
            closest_object = Converted_Distance_Center;
        }

        // Calculate the set point as the average of the left and right sensor distance readings
        Set_Point = closest_object;
    }
    else
    {
...
void Controller_2()
{
    uint32_t STOP_CONDITION = 1;

    // Check if both the left and right distance sensor readings are greater than the desired distance
      if ((Converted_Distance_Left > DESIRED_DISTANCE) && (Converted_Distance_Right > DESIRED_DISTANCE) )
      {
          if((Converted_Distance_Left > TOO_FAR_DISTANCE) && (Converted_Distance_Right > TOO_FAR_DISTANCE)) {
              STOP_CONDITION = 0;
          }
      ...
      // Calculate the new duty cycle for the right motor based on the error and proportional constant (Kp)
      Duty_Cycle_Right = (PWM_NOMINAL*STOP_CONDITION) - (Kp * Error);

      // Calculate the new duty cycle for the left motor based on the error and proportional constant (Kp)
      Duty_Cycle_Left  = (PWM_NOMINAL*STOP_CONDITION) + (Kp * Error);
